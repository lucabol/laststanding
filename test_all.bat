...
for %%f in (bin\test.exe bin\test_strings.exe bin\test_fs.exe bin\test_utils.exe bin\test_img.exe bin\test_tls.exe bin\test_clipboard.exe bin\gfx_test.exe bin\ui_test.exe) do (
    if exist "%%f" (
        echo --- Running %%f ---
        "%%f"
        if !ERRORLEVEL! neq 0 (
