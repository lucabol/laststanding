...
#ifdef L_WITHSOCKETS
static int open_loopback_listener(L_SOCKET *out_server, int *out_port) {
...
static int open_loopback_udp_socket(L_SOCKET *out_sock, int *out_port) {
...
void test_sockets(void) {
    ...
    TEST_FUNCTION("l_socket_connect hostname / loopback");
    {
        L_SOCKET server = -1;
        int port = 0;
        if (open_loopback_listener(&server, &port) == 0) {
...
void test_udp(void) {
    ...
    TEST_FUNCTION("l_socket_sendto hostname / loopback");
    {
        L_SOCKET receiver = -1;
        int port = 0;
        if (open_loopback_udp_socket(&receiver, &port) == 0) {
